...
# IB Gateway API connection (4001 = live, 4002 = paper). Use different client ID than Send_Live_Trade_GW.
IB_HOST = "127.0.0.1"
...
def read_exit_trade_info():
    """
    Read Live_Trade_Info.xlsx (A, B, C, D). Column D = IB Exit (written by Stage_Trades_Auto).
    ...
    for row in range(2, ws.max_row + 1):
        ...
        exit_type_cell = ws.cell(row=row, column=4).value
...
def main():
    try:
        exits, all_are_open = read_exit_trade_info()
